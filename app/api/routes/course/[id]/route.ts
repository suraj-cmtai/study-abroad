...
export async function PUT(
    ...
    try {
        ...
        if (modules) courseData.modules = modules.toString().split('.').map(s => s.trim()).filter(Boolean);
        if (prerequisites) courseData.prerequisites = prerequisites.toString().split('.').map(s => s.trim()).filter(Boolean);
        if (careerOpportunities) courseData.careerOpportunities = careerOpportunities.toString().split('.').map(s => s.trim()).filter(Boolean);
